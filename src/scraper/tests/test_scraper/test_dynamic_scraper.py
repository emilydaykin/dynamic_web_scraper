...
def test_dynamic_imdb_search(mock_imdb_search_term):
    ...
    # this could change if the site changes
    assert len(results) > 3, 'There should be more than 3 results'
...
def test_dynamic_lonely_planet_search(mock_lonely_planet_search_term):
    ...
    scraper = Scraper()
    results = scraper.scrape_lonely_planet_search(
        *mock_lonely_planet_search_term)
    assert type(results) == list
    assert len(results) > 10  # this could change if site changes
    for term in mock_lonely_planet_search_term:
        assert all([term in result for result in results]
                   ), f'Not all results have "{term}" in url.'
